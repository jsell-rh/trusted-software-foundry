...
func scaffoldCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "scaffold",
		Short: "Generate a starter app.tsc.yaml for a new application",
		Long: `scaffold generates a valid app.tsc.yaml with all 7 trusted components pinned
and placeholder resource definitions. AI agents use this as a starting point
when creating a new application — edit the YAML, then run tsc compile.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Println("Next: edit the spec, then run: tsc compile", outputFile, "-o ./out/")
			return nil
		},
	}
