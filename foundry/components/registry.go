...
func New(catalog ...AuditRecord) *Registry {
	...
	for _, rec := range catalog {
		...
		if _, exists := cat[k]; exists {
			panic(fmt.Sprintf("foundry/components: duplicate catalog entry %s", k))
		}
		if rec.Name == "" {
			panic("foundry/components: catalog entry has empty Name")
		}
		if rec.Version == "" {
			panic("foundry/components: catalog entry has empty Version")
		}
		if rec.Hash == "" {
			panic("foundry/components: catalog entry has empty Hash")
		}
		cat[k] = rec
	}
	return &Registry{
...
func (r *Registry) Register(c spec.Component) error {
	...
	rec, ok := r.catalog[k]
	if !ok {
		return fmt.Errorf("foundry/components: %s is not in the audit catalog", k)
	}
	if hash != rec.Hash {
		return fmt.Errorf("foundry/components: audit hash mismatch for %s: got %s, want %s", k, hash, rec.Hash)
	}
	if _, exists := r.components[name]; exists {
		return fmt.Errorf("foundry/components: component %q is already registered", name)
	}
...
func HashSourceDir(dir string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("foundry/components: hashing dir %s: %w", dir, err)
	}
