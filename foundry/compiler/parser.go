...
// defaultSchemaPath returns the path to foundry/spec/schema.json resolved relative
// to this source file so it works regardless of working directory.
func defaultSchemaPath() string {
	_, file, _, _ := runtime.Caller(0)
	// file is .../foundry/compiler/parser.go; schema is .../foundry/spec/schema.json
	return filepath.Join(filepath.Dir(file), "..", "spec", "schema.json")
}

// Parse reads, parses, and validates an IR spec file (app.foundry.yaml).
// Validation runs in two passes:
//  1. JSON Schema structural validation against foundry/spec/schema.json
