...
// expandEnvVar converts an IR env-var reference like "${MY_VAR}" into a Go
// expression `env("MY_VAR", "")`. If the value is not a ${…} reference it
// is returned as a quoted Go string literal.
...
var mainGoTemplate = template.Must(template.New("main").Funcs(template.FuncMap{
	...
	// envVar expands a spec env-var reference (${VAR}) to a Go env() call.
	"envVar": expandEnvVar,
	// goStr produces a properly-escaped Go string literal for any string value.
	// Using fmt.Sprintf("%q", s) ensures special characters (quotes, backslashes,
	// newlines, etc.) are safely escaped, preventing malformed generated Go code
	// even when spec values contain unexpected characters.
...
{{ end }}{{ if .IR.Observ.Metrics }}		"foundry-metrics": {"port": {{ .IR.Observ.Metrics.Port }}, "path": {{ goStr .IR.Observ.Metrics.Path }}},
...
{{ end }}{{ end }}{{ if .IR.Tenancy }}		"foundry-tenancy": {"field": {{ goStr .IR.Tenancy.Field }}, "strategy": {{ goStr .IR.Tenancy.Strategy }}, "header": {{ goStr .IR.Tenancy.Header }}},
{{ end }}	}
